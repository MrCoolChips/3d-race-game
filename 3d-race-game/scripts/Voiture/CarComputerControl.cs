...
{
    [RequireComponent(typeof (AdversaireController))]
    public class CarComputerControl : MonoBehaviour
    {
        ...
        private AdversaireController m_CarController;    // Reference to actual car controller we are controlling
        ...
        private void Awake()
        {
            ...
            m_CarController = GetComponent<AdversaireController>();
        ...
        IEnumerator RevenirEnArriere() {
            m_Driving = false;
            yield return new WaitForSeconds(0.2f);
            m_Rigidbody.velocity = -transform.forward * 10f;
            m_Rigidbody.velocity += transform.right * 2f;
            yield return new WaitForSeconds(3f);
            m_Driving = true;
        }

        IEnumerator TournerLeVehicule() {
            m_Driving = false;
        ...
        private void OnTriggerEnter(Collider other)
        {
            if (other.CompareTag("Obstacles")) {
                StartCoroutine(RevenirEnArriere());
            }
            if (other.CompareTag("Routes")) {
                StartCoroutine(TournerLeVehicule());
            }
        }
        ...
        private void OnCollisionStay(Collision col)
        {
            ...
            {
                var otherAI = col.rigidbody.GetComponent<CarComputerControl>();
                if (otherAI != null)
                {
